...
#define MEM(reg, offset) MemOperand(reg, offset)
      ...
      {
        //_ Mov(TMP_REG_0, memory_address); // should we replace with `Ldr` to set  X17 ?
        _ AdrpAdd(TMP_REG_0, curr_relo_pc, memory_address);
        if (opc == 0b00)
          _ ldr(W(rt), MEM(TMP_REG_0, 0));
        else if (opc == 0b01)
          _ ldr(X(rt), MEM(TMP_REG_0, 0));
      ...
      {
        //_ Mov(X(rd), runtime_address); // should we replace with `Ldr` to set  X17 ?
        _ AdrpAdd(X(rd), curr_relo_pc, runtime_address);
      }
      debug_nop();
      ...
      {
        //_ Ldr(TMP_REG_0, branchAddressLabel); // should we replace with `Mov` to set  X17 ?
        _ AdrpAdd(TMP_REG_0, curr_relo_pc, branch_address);
        if ((instr & UnconditionalBranchMask) == BL) {
          _ blr(TMP_REG_0);
        } else {
          _ br(TMP_REG_0);
  ...
  _ AdrpAdd(TMP_REG_0, curr_relo_pc, curr_orig_pc);
  _ br(TMP_REG_0);
...
int GenerateCheckCode(void *buffer, void *address, void *data) {
  TurboAssembler turbo_assembler_((void *)address);
#define _ turbo_assembler_.

  int64_t offset = (uint64_t)data - (uint64_t)address;
  _ ldr(TMP_REG_0, offset / 4);
  _ Emit(0xB4000051); //cbz x17, #8
  _ br(TMP_REG_0);

  CodeBufferBase *trampoline_buffer = turbo_assembler_.GetCodeBuffer()->Copy();
  memcpy(buffer, trampoline_buffer->getRawBuffer(), trampoline_buffer->getSize());
  return trampoline_buffer->getSize();
}
...
#define _ turbo_assembler_.
  ...
  uint64_t from = (uint64_t)src;
  uint64_t to = (uint64_t)dst;
  uint64_t distance = to - from;
  printf("b %p=>%p\n", from, to);
  if (llabs(distance) < ARM64_B_XXX_RANGE) {
    _ b(distance);
  } else {

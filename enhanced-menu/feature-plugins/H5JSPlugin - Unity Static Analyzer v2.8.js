        ...
        if (Math.abs(level) < maxHops) {
            ...
            for (let parent_class_name in window.gUnityClasses) {
                ...
                for (let parent_field_name in parent_fields) {
                    ...
                    if (parent_field_type === class_name && !visited.has(parent_class_name)) {
                        visited.add(parent_class_name);
                        queue.push({
                            class_name: parent_class_name,
                            level: level - 1
                        });
                        //TODO: currently cannot handle class with multiple field of interested class, only stored first copy
...
/*
if (Network.prototype.hasOwnProperty('addNode')) {
  console.log('The addNode() method exists on the Network class');
}
...
function showunitypluginclasspage(selectedClass) {
    ...
    renderChildGraphPromise(selectedClass)
    renderParentGraphPromise(selectedClass)
    //attachNetworkEvent()
}
...
function refreshOnSceneObjectList() {
    ...
    /*resultAry = objectAry.map(function(element) {
       return ["direct", element, 0, "", ""];
    });*/
    resultObj["direct"] = {
        parentName: "direct",
        connectedFieldName: "",
        connectedFieldOffset: "",
        level: 0,
    ...
    for (var i = -1; i > -5; i--) {
        ...
        for (var j = 0; j < parentNodes.length; j++) {
            ...
            //for now, just concat them all together first
            //objectAry = objectAry.concat(parentObjectAry);
            /*let tmpAry = parentObjectAry.map(function(element) {
               return [parentNodes[j].id, element, i, parentNodes[j].fieldName, parentNodes[j].offset];
            });*/
            let tmpObj = {
                parentName: parentNodes[j].id,
                connectedFieldName: parentNodes[j].fieldName,
                connectedFieldOffset: parentNodes[j].offset,
                level: i,
                objectList: parentObjectAry,
            }
            //resultAry = resultAry.concat(tmpAry);
            resultObj[parentNodes[j].id] = tmpObj
        }
    }

    //let tmp;
    /*for (let i = 0; i < objectAry.length; i++) {
        if (!tmp)
            tmp = '<a href=# onclick="onShowUnityObjInfo(true,' + objectAry[i] + ',true)">' + objectAry[i] + '</a>';
        else
            tmp = tmp + ', ' + '<a href=# onclick="onShowUnityObjInfo(true,' + objectAry[i] + ',true)">' + objectAry[i] + '</a>';
    }*/
    /*
        for (let i = 0; i < resultAry.length; i++) {
            if (!tmp)
                tmp = '<a href=# onclick="onShowUnityObjInfo(true,' + resultAry[i][1] + ',true)">' + resultAry[i][1] + '</a>';
            else
                tmp = tmp + ', ' + '<a href=# onclick="onShowUnityObjInfo(true,' + resultAry[i][1] + ',true)">' + resultAry[i][1] + '</a>';
        }
    */
    //$('table#unitystaticdetailinfolist #onSceneObjectList').html(tmp);
    let row = "";
    let keys = Object.keys(resultObj);
    for (var i = 0; i < keys.length; i++) {
        let tmp;
        let resultAry = resultObj[keys[i]].objectList
        if (resultAry.length == 0) continue;
        for (var j = 0; j < resultAry.length; j++) {
            let tmpObj = getTargetObj(resultObj[keys[i]], resultAry[j])
            if (tmpObj == 0) continue;
            if (!tmp)
                tmp = '<a href=# onclick="onShowUnityObjInfo(true,' + tmpObj + ',true)">' + tmpObj + '</a>';
            else
                tmp = tmp + ', ' + '<a href=# onclick="onShowUnityObjInfo(true,' + tmpObj + ',true)">' + tmpObj + '</a>';
        }
        row += '<tr><td>&#8658;' + keys[i] + '(' + resultObj[keys[i]].level + ')&#8658;<br>'
        row += '<span style="font-size: 70%;"> - ' + resultObj[keys[i]].connectedFieldName + ' (' + resultObj[keys[i]].connectedFieldOffset + ')</span></td><td>' + tmp + '</td><tr>'
    }
    $("table#unitystaticdetailinfolist tbody").append(row);

    function getTargetObj(infoObj, curUnityObj) {
        if (infoObj.level == 0) return curUnityObj;
        else if (infoObj.level == -1) return "0x" + readPtr(Number(curUnityObj) + Number(infoObj.connectedFieldOffset)).toString(16);
        else if (infoObj.level <= -2) {
            //get level+1 object
            var nextLevelObj = readPtr(Number(curUnityObj) + Number(infoObj.connectedFieldOffset));
            //get edge to connect level+1 node 
            var tmpEdges = network.getConnectedEdges(infoObj.parentName, function (tmpEdge) {
                //should only give one edge
                return tmpEdge.from === infoObj.parentName;
            });
            //get level+1 node 
            var nextLevelNode = network.body.edges[tmpEdges[0]].to;
            if (infoObj.level == -2)
                return "0x" + readPtr(Number(nextLevelObj) + Number(nodes.get(nextLevelNode.id).offset)).toString(16)
            else {
                var nextLevelInfoObj = resultObj[nextLevelNode.id]
                //alert(nextLevelNode.id+":"+infoObj.parentName+":"+nodes.get(nextLevelNode.id).offset+":"+nextLevelObj+":"+JSON.stringify(nextLevelInfoObj))
                return getTargetObj(nextLevelInfoObj, nextLevelObj)
            }
        } /*else if (infoObj.level<-2) {
          var nextLevelObj = readPtr(Number(curUnityObj)+Number(infoObj.connectedFieldOffset));
       }*/
    ...
    function extractObjFromFields(SourceObj, SourceObjKlass, TargetObjKlass) {
        let resultObj = []

...
function showlog(msg)
{
    ...
    if(!window.stopAutoSearchOffset) {
        html += "<br/><button onclick='window.stopAutoSearchOffset=true;'>停止</button>";
        if(window.pauseAutoSearchOffset)
            html += "<button onclick='window.pauseAutoSearchOffset=false;'>继续</button>";
        else
            html += "<button onclick='window.pauseAutoSearchOffset=true;'>暂停</button>";
    }

    html += "<br/><br/>已找到"+FoundChains.length+"条偏移链";
    ...
    if(window.stopAutoSearchOffset) html += "<br/><br/><button onclick='window.closeAutoSearchOffset()'>关闭</button>";
...
for(var level=0; level<maxLevel; level++)
{
    ...
    for(var i=0; i<LevelResults.length; i++)
    {
        ...
        await sleep(100);
        showlog("搜索第"+(level+1)+"层第"+(i+1)+"/"+LevelResults.length+"个指针=>"+gLevelResults.length+"+"+count);
